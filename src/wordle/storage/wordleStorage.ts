...
// ── Practice (Quick Play) in-progress autosave ──
// Same as the daily save but not date-bound — persists the current practice
// game so it can be resumed after backgrounding or force-quitting the app.
// Cleared when the game ends (won or lost) or a new practice game begins.
const PRACTICE_PROGRESS_KEY = "wordle_practice_progress_v1";

export type WordlePracticeProgress = {
  solution: string;
  guesses: string[];
  evaluations: any[][];
  currentGuess: string;
...
export async function loadPracticeProgress(): Promise<WordlePracticeProgress | null> {
  try {
    const raw = await AsyncStorage.getItem(PRACTICE_PROGRESS_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed.solution !== "string") return null;
    return parsed;
  } catch (e) {
...
export async function savePracticeProgress(progress: WordlePracticeProgress): Promise<void> {
  try {
