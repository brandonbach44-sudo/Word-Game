// src/shared/GameTile.tsx
// Renders a single tiered/cosmetic letter tile — metal-gradient tiers
// (Copper–Gold), gem-image tiers (Ruby–Rose Quartz), and the free
// Default/Classic tiers. Shared across every game that uses individual
// letter tiles (Word Builder, Anagrams) so an equipped skin looks and
// animates identically no matter which game you're playing.
...
import { TierName } from './tileTiers';
...
// Classic tier style colors (6 options) - Softer, lighter colors.
// White used to be a 7th option here; removed since it read as blank/empty
// rather than as an intentional color choice.
...
export const GameTile = ({
  ...
  // Note: variant 3 ("more intense glow") was removed — tiers only ever go
  // up to V2 now (see TierPreviewPanel, which dropped its V3 unlock path).
  ...
  // 'default' is always the app's cream tile + border — a fixed look,
  // independent of whichever background theme the page has equipped, so
  // it never blends in and disappears against a dark theme. 'classic' uses
  // its own 6 color styles below.
  if (tierName === 'default') {
    return (
      <TouchableOpacity onPress={() => onPress(index)} activeOpacity={0.8}>
        <View style={[styles.tileContainer, { width: tileSize, height: tileSize }]}>
          <View style={[styles.tile, { width: tileSize, height: tileSize, backgroundColor: '#f5f0e6', borderColor: '#d8cdb4', borderWidth: 1.5 }]}>
            <Text style={[styles.letterText, { fontSize: Math.round(tileSize * 0.55), color: '#2c2416' }]}>{letter}</Text>
          </View>
        </View>
      </TouchableOpacity>
    );
